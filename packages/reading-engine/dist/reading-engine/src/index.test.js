...
import { renderVolumeIV, sumToCoreNumber, } from "./index";
...
describe("renderVolumeIV", () => {
    ...
    it("returns all delivery blocks when requested", () => {
        const result = renderVolumeIV({ coreNumber: 4, tokens: [] }, { format: "blocks", explain: true });
    ...
    it("stays silent about temperature without qualified tokens", () => {
        ...
        expect(renderVolumeIV({ coreNumber: 7, tokens: withoutTemp }).blocks.resonance.toLowerCase()).not.toContain("temperature");
        expect(renderVolumeIV({ coreNumber: 7, tokens: lowConfidence }).blocks.resonance.toLowerCase()).not.toContain("temperature");
        expect(renderVolumeIV({ coreNumber: 7, tokens: withTemp }).blocks.resonance.toLowerCase()).toContain("temperature cue 72°F".toLowerCase());
    });
    ...
    it("produces deterministic mappings for supported cores", () => {
        const coreKeys = Object.keys(CORE_EXPECTATIONS)
            ...
            .filter((core) => core in CORE_EXPECTATIONS);
    ...
    it("switches tone vocab without changing logical gates", () => {
        ...
        const direct = renderVolumeIV({ coreNumber: 3, tokens: analyticsTokens }, { tone: "direct" });
        const encouraging = renderVolumeIV({ coreNumber: 3, tokens: analyticsTokens }, { tone: "encouraging" });
    ...
    it("matches snapshot for calm tone delivery", () => {
        const result = renderVolumeIV({ coreNumber: 8, tokens: analyticsTokens }, { format: "blocks", tone: "calm" });
        expect(result.blocks).toMatchSnapshot();
    });
    it("matches snapshot for master 11 without auxiliary tokens", () => {
        const result = renderVolumeIV({ coreNumber: 11, tokens: [] }, { format: "blocks", tone: "encouraging" });
        expect(result.blocks).toMatchSnapshot();
    });
});
