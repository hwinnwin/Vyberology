...
describe("generateReadingV4", () => {
    ...
    it("throws when the feature flag is disabled", () => {
        const input = { raw: "15:51 74%", entryNo: 60 };
    ...
    it("produces the mirror reading for 15:51 + 74%", () => {
        ...
        expect(reading.header.title).toBe("Cycle IV Entry #60 — 15:51 Seal (The Mirror of Renewal)");
        expect(reading.header.theme).toEqual([
            "Transition",
            "Balance",
            "Return to Self",
        ]);
        expect(reading.anchorFrame.time).toContain("15:51 — a mirrored time sequence — appears during moments of completion and grounding.");
        ...
        expect(reading.energyMessage).toBe("As you arrive home, so does your energy. The mirror closes — what you set in motion now grounds into your world.");
        ...
        expect(reading.essenceSentence).toBe("15:51 — balance returns as movement finds its calm.");
    });
    it("produces the gateway reading for 11:11", () => {
        ...
        expect(reading.header.title).toBe("Cycle IV Entry #57 — 11:11 Seal (Alignment Gateway)");
        expect(reading.header.theme).toEqual([
            "Awakening",
            "Confirmation",
        ...
        expect(reading.energyMessage).toBe("The door is open — act with calm certainty.");
        ...
        expect(reading.essenceSentence).toBe("11:11 — alignment confirmed; act with calm certainty.");
    });
    it("produces the arrival reading for 77 km + 11.1 L", () => {
        ...
        expect(reading.header.title).toBe("Cycle IV Entry #47 — 77 km · 11.1 L Seal (Home Frequency)");
        // added by Claude Code (Stage 3) — updated to match new phrasebook energy message
        expect(reading.energyMessage).toBe("You've arrived within yourself; wisdom is grounded now.");
        expect(reading.resonance.blurb).toBe("Divine awareness anchored into stability.");
        ...
        expect(reading.essenceSentence).toBe("77 km · 11.1 L — wisdom grounded; you've arrived where peace meets purpose.");
        expect(reading.alignmentSummary[0].focus).toBe("Reflection");
    });
});
describe("assembleReading", () => {
    it("returns explain payload when requested", () => {
        const input = { raw: "06:06 71%", entryNo: 63 };
    ...
    it("maps layered meaning for time, percent, temperature, and count tokens", () => {
        const input = { raw: "06:16 99% 18°C 1200", entryNo: 12 };
        ...
        expect(reading.layeredMeaning.find((row) => row.segment === "18°C")?.essence).toContain("Temperature cue");
        expect(reading.layeredMeaning.find((row) => row.segment === "99%")?.essence).toContain("Near completion");
    });
