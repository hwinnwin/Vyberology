...
describe("parse and motif helpers", () => {
    it("extracts and normalises all token types with buckets", () => {
        const tokens = extractTokens("05:56 99% 18°C 12 km 6.7 L/100km 8.4 L 155000 CT999 88%", {
            percentThresholds: {
                nearFull: 98,
                seventies: [70, 79],
            },
        ...
        expect(tokens.some((token) => token.type === "percent" && token.bucket === "near_full")).toBe(true);
        expect(tokens.some((token) => token.type === "percent" && token.bucket === "88")).toBe(true);
        expect(distance?.value).toMatchObject({ value: 12, unit: "km" });
        const hasFuelValue = tokens.some((token) => token.type === "fuel" &&
            Math.abs(token.value.value - 8.4) < 0.001);
        ...
        expect(efficiencyTokens.some((token) => token.type === "consumption")).toBe(true);
    });
...
describe("assembly fallbacks", () => {
    ...
    it("maps layered meaning for temperature tokens", () => {
        const input = { raw: "05:56 18°C", entryNo: 2 };
    ...
    it("deduplicates alignment summary entries for repeated digits", () => {
        const input = { raw: "15:51 30%", entryNo: 8 };
        const { reading } = assembleReading(input);
    ...
    it("falls back to token-only essence when templates are missing", () => {
        ...
        const input = { raw: "15:51", entryNo: 7 };
        const { reading } = assembleReading(input, {
            config: {
                phrasebook: cloned,
                thresholds: { nearFullPercent: 95, seventies: [70, 79] },
