...
export function detectMotifs(tokens, context) {
    const detected = new Set();
    const timeTokens = tokens.filter((token) => token.type === "time");
    tokens.forEach((token) => {
        const tokenFlags = [];
        if (token.type === "time" && isMirrorTime(token.value.iso)) {
...
function dedupeMotifs(flags) {
    return Array.from(new Set(flags));
}
function isMirrorTime(iso) {
...
function isBuilderPattern(raw) {
    const digits = raw.replace(/\D/g, "");
    return digits.includes("44") || digits.includes("1144");
}
function isAbundance(raw) {
    const digits = raw.replace(/\D/g, "");
    return digits.includes("88") || /^8+$/.test(digits);
}
function isProgression(timeTokens) {
    const minutesList = timeTokens
        .map((token) => token.value.totalMinutes)
        .sort((a, b) => a - b);
...
function isArrival(tokens, context) {
    const hasDistance = tokens.some((token) => token.type === "distance");
    const hasFuelOrConsumption = tokens.some((token) => token.type === "fuel" || token.type === "consumption");
    ...
    return Boolean(distanceToken &&
        "value" in distanceToken.value &&
        distanceToken.value.value === 77);
}
export function motifStrength(motif) {
    return MOTIF_PRIORITY.indexOf(motif);
}
export function dominantMotif(tokens, motifs) {
    if (!motifs.length) {
        return undefined;
    }
