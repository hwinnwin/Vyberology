...
export function assembleReading(input, options) {
    ...
    const reading = {
        header: buildHeader(input, orderedTokens, mergedConfig.phrasebook, primaryMotif, coreFrequency, headline),
        ...
        energyMessage: pickEnergyMessage(mergedConfig.phrasebook, primaryMotif, coreFrequency),
        alignmentSummary: buildAlignmentSummary(orderedTokens, mergedConfig.phrasebook, coreFrequency),
        resonance: pickResonance(mergedConfig.phrasebook, primaryMotif, coreFrequency),
        guidanceAspect: pickGuidanceAspect(mergedConfig.phrasebook, primaryMotif, coreFrequency),
        essenceSentence: pickEssenceSentence(mergedConfig.phrasebook, primaryMotif, orderedTokens, headline),
    };
    let explain;
    if (options?.explain) {
...
function deriveCoreFrequency(tokens) {
    const seed = tokens.reduce((acc, token) => acc + valueForCore(token), 0);
...
function buildHeader(input, tokens, phrasebook, motif, core, headline) {
    ...
    const themeList = themes?.[coreKey] ?? phrasebook.themes.default?.[coreKey] ?? [];
    return { title, theme: themeList };
}
function buildAnchorFrame(tokens, phrasebook, motif) {
    const anchors = {};
    const motifKey = motif ?? "default";
    tokens.forEach((token) => {
        ...
        const template = templateGroup[motifKey] ??
...
function buildLayeredMeaning(tokens, phrasebook) {
    const layered = [];
...
function lookupLayerEntry(phrasebook, token) {
...
function buildAlignmentSummary(tokens, phrasebook, core) {
    ...
    if (rows.length < 4) {
        const digitKeys = Array.from(new Set(tokens.flatMap((token) => token.reduction.digits.map((digit) => String(digit)))));
...
function pickResonance(phrasebook, motif, core) {
    const motifKey = motif ?? "";
    const byMotif = phrasebook.resonance.byMotif?.[motifKey];
    if (byMotif) {
        return byMotif;
    }
    return (phrasebook.resonance.byCore[String(core)] ??
        phrasebook.resonance.byCore["5"]);
}
function pickGuidanceAspect(phrasebook, motif, core) {
    const motifKey = motif ?? "default";
...
function pickEssenceSentence(phrasebook, motif, tokens, headline) {
    const motifKey = motif ?? "default";
    const template = phrasebook.essenceSentence[motifKey] ??
        phrasebook.essenceSentence.default ??
        "{token}";
    const primaryToken = headline.length > 0 ? headline : tokens[0]?.raw ?? "This moment";
    return template.replace("{token}", primaryToken);
}
function selectHeadline(tokens, motif) {
    const motifTokens = motif
        ? tokens.filter((token) => token.flags.includes(motif))
        : [];
    const headlineTokens = motifTokens.length > 0 ? motifTokens : tokens.slice(0, 2);
    const uniqueSegments = Array.from(new Set(headlineTokens.map((token) => token.raw)));
    if (uniqueSegments.length === 0) {
        return tokens[0]?.raw ?? "Snapshot";
    }
    return uniqueSegments.slice(0, 2).join(" · ");
}
function buildExplainPayload(input, reading, motif) {
    const explainInput = {
        raw: input.raw,
