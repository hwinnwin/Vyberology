...
import { ArrowLeft, Home, Sparkles, Crown, Lock, Check } from "lucide-react";
...
export default function LyfPath() {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    void trackAnalyticsEvent("lyf_path_free_calculated", {
      platform: "web",
  ...
  const handlePaidGeneration = async (tier: ReadingTier) => {
    ...
    try {
      ...
      const readingText = await callVybeReading(inputs, depth, "lyf-path");
      void refreshCredits();

      // Save to localStorage (legacy)
      saveReading({
        inputType: "manual",
        inputValue: `Lyf Path: ${freeResult.fullName}, DOB ${dob} (${tier})`,
        reading: readingText,
      });
      ...
      void trackAnalyticsEvent("lyf_path_paid_generated", {
        platform: "web",
        tier,
        lifePathNumber: numbers.lifePath.value,
        isMaster: numbers.lifePath.isMaster,
      ...
      toast({ title: "Reading Complete", description: "Your reading is ready." });
  ...
  return (
    <div className="min-h-screen flex flex-col bg-vy-parchment grain">
      ...
      <header className="text-center px-6 pt-12 pb-8">
        <p className="vy-label text-vy-gold mb-4">Soul Blueprint</p>
        ...
        <p className="font-sans text-base font-light text-vy-charcoal/50 max-w-[440px] mx-auto">
          Your name and birth date hold the key to your base frequency — the note your soul chose to vibrate at
        </p>
      </header>
        ...
        <form onSubmit={handleCalculate} className="space-y-4">
          <div>
            ...
            <p className="font-sans text-xs text-vy-charcoal/40 mt-1.5 italic">
              Use your full name as it appears on your birth certificate, including middle names, for best accuracy.
            </p>
          </div>
          <div>
      ...
      {freeResult && !isGenerating && (
        <section className="max-w-[720px] mx-auto px-6 pb-12 w-full animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
          ...
          <div className="relative mb-10 rounded-2xl border border-vy-charcoal/[0.08] bg-white/30 overflow-hidden">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-vy-parchment/60 backdrop-blur-[1px]">
              ...
              <p className="font-sans text-sm font-semibold text-vy-charcoal/70 mb-1">
                Your full Soul Blueprint is ready
              </p>
              <p className="font-sans text-xs text-vy-charcoal/40">
                Deep interpretations, frequency mapping, and AI-powered guidance
              </p>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            >
              <div className="flex items-center gap-2 mb-3 text-vy-charcoal/70">
                ...
                <span className="font-sans text-sm font-semibold text-vy-charcoal">{lyfPathTier.label}</span>
              </div>
              <p className="font-sans text-xs text-vy-charcoal/50 mb-4">{lyfPathTier.tagline}</p>

              <ul className="space-y-2 mb-5">
                {lyfPathTier.features.map((f, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3 text-vy-charcoal/70">
                ...
                <span className="font-sans text-sm font-semibold text-vy-charcoal">{fullVybeTier.label}</span>
              </div>
              <p className="font-sans text-xs text-vy-charcoal/50 mb-4">{fullVybeTier.tagline}</p>

              <ul className="space-y-2 mb-5">
                {fullVybeTier.features.map((f, i) => (
                  <li key={i} className="flex items-start gap-2">
                    <Check className="w-3.5 h-3.5 text-vy-gold flex-shrink-0 mt-0.5" />
                    <span className="font-sans text-xs text-vy-charcoal/70">{f}</span>
                  </li>
          ...
          <div className="sm:hidden fixed bottom-0 left-0 right-0 p-4 bg-vy-parchment/95 backdrop-blur-md border-t border-vy-charcoal/[0.08] z-40">
            ...
            >
              Unlock Full VYBE Reading — {fullVybeTier.price}
            </button>
          </div>
        </section>
      )}
      ...
      {isGenerating && (
        <div className="flex justify-center py-12">
          <LoadingSpinner size="lg" text="Channeling your reading..." />
        </div>
      )}
