...
export const VoiceAssistant = ({ onCommand, isActive = false, disabled = false }: VoiceAssistantProps) => {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      // Check for wake word "hey lumyn" (speech recognition may transcribe as "lumen")
      if (fullTranscript.includes('hey lumen') || fullTranscript.includes('hi lumen') || fullTranscript.includes('hey lumyn') || fullTranscript.includes('hi lumyn')) {
        setWakeWordDetected(true);
        setIsProcessing(true);

        toast({
          title: "👋 Hey there!",
          description: "Lumyn is listening... What's the vybe?",
        });

        // Extract command after wake word
        const commandStart = Math.max(fullTranscript.indexOf('lumyn'), fullTranscript.indexOf('lumen')) + 5;
  ...
  const startListeningAfterPermission = () => {
    if (recognitionRef.current && !isListening) {
      try {
        ...
        toast({
          title: "Listening...",
          description: 'Say "Hey Lumyn" to activate',
        });
      } catch (e) {
        if (import.meta.env.DEV) {
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="relative">
          <div className={`relative grid place-items-center rounded-full bg-gradient-to-br from-lf-violet to-lf-aurora p-4 ${
            ...
            <img src={vybeLogo} alt="Lumyn" className="h-16 w-16 relative z-10" />
          </div>
        </div>
        ...
        <div className="text-center">
          <h3 className="font-display text-xl font-semibold text-white mb-2">
            {wakeWordDetected ? "I'm listening!" : isListening ? "Say 'Hey Lumyn'" : "Voice Assistant"}
          </h3>
          ...
          {!isListening && (
            <p className="text-sm text-lf-slate">
              Activate voice commands with "Hey Lumyn"
            </p>
          )}
        </div>
        ...
        <div className="w-full mt-4 p-4 rounded-lg bg-lf-midnight/50 border border-lf-violet/20">
          ...
          <ul className="text-xs text-lf-slate space-y-1">
            <li>"Hey Lumyn, what's the vybe?" - Capture current time</li>
            <li>"Hey Lumyn, read [number]" - Get reading for specific number</li>
            <li>"Hey Lumyn, show history" - View reading history</li>
          </ul>
        </div>
      </div>
            ...
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <div>
                ...
                <p className="text-lf-slate text-sm">
                  Vyberology needs access to your microphone to use the "Hey Lumyn" voice assistant.
                </p>
              </div>
